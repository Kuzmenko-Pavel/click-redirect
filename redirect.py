...
def redirect(environ, start_response):
    ...
    def _redirect_to(url, cookie):
        ...
        response_headers = [('Location',   url),
                            ('Connection', 'close'),
                            ('set-cookie','yottos_unique_id='+ cookie +'; Path=/; Version=1; Max-Age=31536000; Domain=.yottos.com; HttpOnly;')]
    ...
    try:
        ...
        if not valid:
            print "IP %s, cookie %s, token %s, offer id %s, validation -%s-" % (ip, cookie, token, offer_id, params.get('rand',''))
        ...
        print e
        return _redirect_to('https://yottos.com/?utm_source=yottos&utm_medium=redirect&utm_campaign=Not%20Valid%20Click', cookie)
    print 'Redirect complit to %s ' % (datetime.datetime.now() - elapsed_start_time).microseconds
    print "=" * 20
    return _redirect_to(url or 'https://yottos.com/?utm_source=yottos&utm_medium=redirect&utm_campaign=Not%20Valid%20Click', cookie)
...
def utmConverter(url, offer_id, campaign_id, inf_id):
    ...
    offer_title = 'yottos-' + offer_info['title'].encode('utf-8').replace(' ', '_').replace('.', '_').replace(',', '_').replace(';', '_').replace('!', '_').replace('?', '_')
    offer_campaign_title = 'yottos-' + offer_info['campaignTitle'].encode('utf-8').replace(' ', '_').replace('.', '_').replace(',', '_').replace(';', '_').replace('!', '_').replace('?', '_')
...
def _get_offer_info(offer_id, campaignId):
    ...
    try:
        ...
        campaign = db.campaign.find_one({'guid': campaignId}, ['title','yottosPartnerMarker','yottosTranslitMarker','yottosHideSiteMarker'])
